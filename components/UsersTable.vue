...
type Pagination = { page: number; perPage: number; totalPages: number; total: number }
type UsersResponse = { success: boolean; users: User[]; pagination?: Pagination | null }

const users = ref<User[]>([])
const loading = ref(false)
const page = ref(1)
const pagination = ref<Pagination>({ page: 1, perPage: 10, totalPages: 1, total: 0 })
...
const pageNumbers = computed(() => Array.from({ length: pagination.value.totalPages }, (_, i) => i + 1))

const handleEdit = (user: User) => alert(`Edit user: ${user.name} (${user.email})`)
const handleDelete = (user: User) => { if (confirm(`Delete ${user.name}?`)) alert(`Deleted ${user.name}`) }
      ...
      <tbody>
        <tr v-for="user in users" :key="user.id" class="border-b hover:bg-gray-50">
          ...
          <td class="py-1 flex gap-2 flex-wrap">
            <button @click="handleEdit(user)" class="px-2 py-1 bg-yellow-400 text-white rounded hover:bg-yellow-500">Edit</button>
            <button @click="handleDelete(user)" class="px-2 py-1 bg-red-500 text-white rounded hover:bg-red-600">Delete</button>
          </td>
        </tr>
      </tbody>
    </table>
